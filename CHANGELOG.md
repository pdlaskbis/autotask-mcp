...
- **`periodType` and `isTaxable` on `autotask_create_quote_item` and `autotask_update_quote_item`.** `periodType` is what makes a quote line recurring — an integer picklist (`1`=One-Time, `2`=Monthly, `3`=Quarterly, `4`=Semi-Annual, `5`=Yearly), read from the live entity metadata (`GET /QuoteItems/entityInformation/fields`, 2026-09-15). Neither tool declared it, so a recurring service could not be quoted through this server at all: the line was created without a period, Autotask accepted it, and it billed as a one-time charge. A $500/month managed-services line became a single $500 charge, with nothing erroring and the difference only visible on an invoice. Existing quotes in the tenant carry `periodType: 2` on their recurring lines, so the field is in active use.
  - This is a **missing capability, not a silent drop**. Unlike the `assignedResourceRoleID` defect, nothing here accepted the field and threw it away — the schema never declared it, so a caller had no way to send it short of `autotask_raw_request`.
  - The update tool got both fields too, which the original patch did not cover. Without it a line created with the wrong `periodType` could not be corrected through the tool at all; the only recovery was deleting the line and re-adding it.
  - Both fields are declared on the `AutotaskQuoteItem` type rather than riding its index signature.
  - 15 tests (`tests/quote-item-period-type.test.ts`). They drive the **tool handler** and assert on the **outgoing HTTP request body**, because the handler forwards a fixed list of argument names to the service — a field can be declared in the schema and still never reach Autotask, so a schema-only assertion would not have caught this. Covers all five picklist values, `isTaxable: false` surviving (a `||`-style forward would swallow it), and an omitted field staying absent rather than being sent as `null`. Verified by mutation: dropping the create forward fails 5, dropping the update forward fails 3, removing the schema blocks fails 5.
...
- **`autotask_create_contact` omitted a field Autotask requires, so every create failed.** Autotask declares `isActive` as required on the Contacts entity and answers a create without it `HTTP 500 "Missing Required Field: isActive."` Confirmed from the entity metadata rather than the error alone — `GET /Contacts/entityInformation/fields` reports `isActive` with `isRequired: true, dataType: integer` (checked 2026-09-15). The field was absent from the tool schema too, so a caller could not supply it even knowing to.
  ...
  - **An explicit `false` is honoured.** A naive `payload.isActive || 1` would silently turn a deliberately-inactive contact active; a test pins that.
  - **Provenance:** same story as the quote-location fix — this existed since 2026-07 only as a build-time `.patch` applied by a Dockerfile in another repository. The deploy path changed on 2026-09-14, the patch stopped being applied, and nothing noticed, because a patch file has no tests. 7 tests now assert on the outgoing payload. Verified by mutation: the unpatched source fails 3, a `||`-style default fails 3, and **the original patch's own implementation fails 2** — it defaulted the value but did not normalise a caller's boolean.
